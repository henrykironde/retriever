...
comma_delimiter = {
    ...
    "script": {
        ...
        "retriever_minimum_version": "2.0.dev",
        "version": "1.0.0",
        "urls": {"comma_delimiter": "http://example.com/comma_delimiter.txt"},
    },
    "expect_out": ["a,b,c", "1,2,3", "4,5,6"],
}
...
tab_delimiter = {
    ...
    "script": {
        ...
        "retriever_minimum_version": "2.0.dev",
        "version": "1.0.0",
        "urls": {"tab_delimiter": "http://example.com/tab_delimiter.txt"},
    },
    "expect_out": ["a,b,c", "1,2,3", "4,5,6"],
}
        ...
        "resources": [
            {
                ...
                "schema": {
                    "fields": [
                        {"name": "a", "type": "char"},
                        {"name": "b", "size": "20", "type": "char"},
                        {"name": "c", "size": "20", "type": "char"},
                    ]
                },
...
crosstab = {
    "name": "crosstab",
    "raw_data": ["a,b,c1,c2", "1,1,1.1,1.2", "1,2,2.1,2.2"],
    "script": {
        "name": "crosstab",
        "resources": [
            {
...
extra_newline = {
    "name": "extra_newline",
    "raw_data": ["col1,col2,col3", 'ab,"e', 'f",cd'],
    "script": {
        "name": "extra_newline",
        "resources": [
            {
        ...
        "resources": [
            {
                ...
                "schema": {
                    "fields": [
                        {"name": "aa", "type": "char"},
                        {"name": "bb", "size": "20", "type": "char"},
                        {"name": "c c", "size": "20", "type": "char"},
                    ]
                },
...
xml_test_parameters = [
    (test, test["expect_out"]) for test in tests if test != csv_latin1_encoding
]
...
def test_postgres_integration(dataset, expected, tmpdir):
    ...
    assert (
        get_output_as_csv(
            dataset, postgres_engine, tmpdir, db=postgres_engine.opts["database_name"]
        )
        == expected
    )
...
def test_mysql_integration(dataset, expected, tmpdir):
    ...
    assert (
        get_output_as_csv(
            dataset, mysql_engine, tmpdir, db=mysql_engine.opts["database_name"]
        )
        == expected
    )
