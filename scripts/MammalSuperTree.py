...
class main(Script):
    def __init__(self, **kwargs):
        ...
        self.tables = {'trees': Table(**{"name": "trees"})}

    def download(self, engine=None, debug=False,):
        if engine.name != "Download Only":
            raise Exception("The mammal-super-tree dataset contains only non-tabular data files, and can only be used with the 'download only' engine.")
